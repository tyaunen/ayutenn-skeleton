...
Ayutennフレームワークのコア実装は以下のディレクトリ構造になっています:

```
/vendor/tyaunen/ayutenn-core/src
├── /config          - 設定管理(Config.php)
├── /database        - データベース操作(DataManager.php, DbConnector.php, QueryResult.php)
├── /requests        - リクエスト処理(Controller.php, Api.php, Model.php, RequestValidator.php)
├── /routing         - ルーティング(Router.php, Route.php, RouteGroup.php, Middleware.php)
├── /session         - セッション管理(AlertsSession.php)
└── /utils           - ユーティリティ(Logger.php, Redirect.php, CsrfTokenManager.php, etc.)
```

各機能の詳細な実装を確認したい場合は、これらのファイルを参照してください。
...
/vendor/tyaunen/ayutenn-core
└── /src              - Ayutennフレームワークのコア実装
```
...
#### `/app/views`
HTMLテンプレートを格納します。
ルーティングから直接呼び出されるか、Controllerから表示されます。
...
- **AlertsSession**: フラッシュメッセージ(成功・エラー通知)
