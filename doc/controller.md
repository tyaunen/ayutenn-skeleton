...
## 格納ディレクトリ
Controllerは、/app/controllerに格納される単一のクラスファイルである必要があります。分類・整理のためにサブディレクトリを作っても構いません。
...
## name()メソッドを実装する
コントローラーのname()メソッドを実装し、一意なコントローラー名を返してください。
この名前は、フォームの入力値を一時保存する際のセッションキーの生成に使用されます。
...
class Login extends Controller
{
    ...
    public function main(): void
    {
        ...
        if(Auth::login($user_id, $password)){
            ...
            AlertsSession::putAlertMessageIntoSession('【エラー】IDかパスワードが違います。');
...
class Login extends Controller
{
    ...
    public function main(): void
    {
        // $this->parameterに、バリデート+キャスト済みのパラメータが格納されています
...
{
    public static function name(): string { return 'my_controller'; }
...
{
    public static function name(): string { return 'search'; }
    ...
    protected string $redirectUrl = '/search-result';
