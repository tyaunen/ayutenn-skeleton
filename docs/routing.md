# Routing (ルート定義)
このファイルに、ルート定義ファイルの作り方を記述します。

## 実装参照

このドキュメントで説明する機能は、以下のファイルで実装されています:

- **ルートクラス**: `/vendor/tyaunen/ayutenn-core/src/routing/Route.php`
- **ルーターク**: `/vendor/tyaunen/ayutenn-core/src/routing/Router.php`
- **グループ化**: `/vendor/tyaunen/ayutenn-core/src/routing/RouteGroup.php`
- **ミドルウェア**: `/vendor/tyaunen/ayutenn-core/src/routing/Middleware.php`
- **実装例**: `/app/routes/route.php`、`/app/routes/middleware/NeedAuth.php`
...
class NeedAuth extends Middleware
{
    public function canOverrideRoute(): bool
    {
        ...
        if (!Auth::isLogined()) {
            // メッセージをセッションに保存
            AlertsSession::putInfoMessageIntoSession('ログインが必要です。');

            // trueを返すとルートが上書きされる
            return true;
        } else {
            // falseを返すと元のルートが実行される
            return false;
        }
    }
}
...
public function canOverrideRoute(): bool
{
    ...
    if (/* 条件を満たさない */) {
        // 必要に応じてメッセージを保存
        AlertsSession::putInfoMessageIntoSession('エラーメッセージ');
