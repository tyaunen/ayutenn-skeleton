...
このファイルに、ayutennフレームワークを使用する際のベストプラクティスと、よくある間違いをまとめます。
...
### ✅ DO: `QueryResult` の `data` プロパティに直接アクセス
...
`QueryResult` クラスの `data` プロパティは public で、getter メソッドはありません。

### ❌ DON'T: 存在しないメソッドを呼ばない
...
そのため `$result->data[0]` で最初の行を取得する必要があります。

## セッション

### ✅ DO: `AlertsSession::getAlerts()` で全メッセージを取得
...
```php
// ❌ 悪い例
$alerts = AlertsSession::getAlertMessages(); // このメソッドは存在しない
$infos = AlertsSession::getInfoMessages();   // このメソッドは存在しない
```

`getAlertMessages()` や `getInfoMessages()` というメソッドは存在しません。
`getAlerts()` で全メッセージを取得し、`alert_type` で振り分けてください。
