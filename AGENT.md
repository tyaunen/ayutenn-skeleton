...
## 実装ルール(ayutennフレームワーク - Strict Edition)
...
### 4. ディレクトリとファイル配置
- **コントローラー**: `/app/controller`
  - `Ayutenn\Core\Requests\Controller`を継承する
  - **重要**: ファイルの末尾で必ず `return new ClassName();` を記述し、インスタンスを返すこと
- **モデル(バリデーション定義)**: `/app/model`
- **データベース操作**: `/app/database`
  - コントローラーに直接SQLを書かず、専用のManagerクラスを使用する
